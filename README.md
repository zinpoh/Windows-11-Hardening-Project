...
gobuster dir -u http://{ip\_victim} -w /usr/share/wordlists/dirb/common.txt -x php,html,txt,bak,zip,log,config,ini
...
foreach ($path in $VULNERABLE\_PATHS) {
...
netsh advfirewall firewall add rule name="Block\_HTTP\_External" dir=in action=block protocol=TCP localport=80
...
curl -I http://{ip\_victim}/Microsoft/
...
Desarrollado con fines educativos | Seguridad Informática
